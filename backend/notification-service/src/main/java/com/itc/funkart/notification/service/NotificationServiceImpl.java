...
import com.itc.funkart.notification.template.MessageBuilderTemplate;
        ...
        if (event.getEmail() != null && !event.getEmail().isEmpty()) {//making sure email field is present etc.
            ...
            try {
                ...
                emailSentCounter.increment();
                    ...
                    FailedToSendEmailException e) {//we're only concerned with this specific error to catch thrown by smtp server, any other error thrown by unrelated events can be handled by global handler
